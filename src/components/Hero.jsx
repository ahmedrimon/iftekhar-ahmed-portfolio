...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      <div
        ref={marqueeRef}
        className="overflow-hidden border-t border-ink/10 py-4 -mx-8 md:-mx-12 px-8 md:px-12 bg-ink"
      >
        <div className="marquee-track flex gap-12 whitespace-nowrap w-max">
          {[...words, ...words].map((word, i) => (
            <span
              key={i}
              className="font-mono text-xs tracking-[0.3em] uppercase text-paper/50"
            >
              {word}
              <span className="mx-6 text-paper/20">·</span>
            </span>
          ))}
        </div>
      </div>
